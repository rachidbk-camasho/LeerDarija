...
(() => {
  ...
  function show(view, smooth=false){ $$('.view').forEach(v=>v.classList.add('hidden')); $('#'+view).classList.remove('hidden'); window.scrollTo({top:0,behavior:smooth?'smooth':'auto'}); }
  ...
  function openLesson(id){
    const l=window.LESSONS.find(x=>x.id===id); if(!l)return; state.current=l; setTab('learn'); $('#lessonTitle').textContent=`${l.emoji} ${l.title}`; $('#lessonLevel').textContent=l.level;
  ...
  $('#back').onclick=()=>{setTab('learn');show('homeView');}; $('#nextQuestion').onclick=newQuestion; $('#restore').onclick=restore; $('#restore2').onclick=restore;
  function goHomeSection(tab, selector){
    setTab(tab); show('homeView');
    requestAnimationFrame(()=>{ const target=$(selector); if(target) target.scrollIntoView({behavior:'smooth',block:'start'}); });
  }
  $$('.tabbar button').forEach(b=>b.onclick=()=>{
    if(b.dataset.tab==='learn'){setTab('learn');show('homeView');}
    if(b.dataset.tab==='store')goHomeSection('store','#storeSection');
    if(b.dataset.tab==='progress')goHomeSection('progress','#progressSection');
  });
  renderAll(); initStore();
})();
